...
/** Cost guard per IP (in-memory — see rate-limiter.ts). */
const LIMIT_PER_DAY = 25;

const TOOL_ID = "ai-caption-generator" as const;

export async function POST(request: Request) {
  const clientKey = getClientKey(request.headers);
  const limited = checkRateLimit(
    `ai-caption:${clientKey}`,
    LIMIT_PER_DAY,
    DAY_MS,
  );
  if (!limited.ok) {
  ...
  const parsed = parseCaptionRequestBody(json);
  if (!parsed.ok) {
    return NextResponse.json(
  ...
  const { system, user: userPrompt } = buildCaptionPrompts(parsed.body);

  try {
    const { text } = await bytezInvokeLlm({ system, user: userPrompt });
    const result = parseCaptionModelJson(text);
    if (!result.ok) {
      await refundToolUse({
        toolId: TOOL_ID,
        user,
    ...
    console.error("[ai-caption-generator]", e);
    return NextResponse.json(
